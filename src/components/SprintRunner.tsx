...
const TIME_THEMES: TimeTheme[] = [
  { bgFrom: '#dbeafe', bgTo: '#fef3c7', cloudColor: '#9ca3af', cloudOpacity: 0.4 },
  { bgFrom: '#e0f2fe', bgTo: '#ffffff', cloudColor: '#d1d5db', cloudOpacity: 0.5 },
  { bgFrom: '#fed7aa', bgTo: '#e9d5ff', cloudColor: '#fb923c', cloudOpacity: 0.6 },
  { bgFrom: '#e9d5ff', bgTo: '#cbd5e1', cloudColor: '#a78bfa', cloudOpacity: 0.5 },
];
...
const playJumpBeep = () => createBeep(600, 0.08, 0.08);
const playMilestoneBeep = () => {
  createBeep(800, 0.15, 0.1);
  setTimeout(() => createBeep(1000, 0.15, 0.1), 100);
};
const playCollectSound = () => {
  createBeep(523, 0.08, 0.08);
  setTimeout(() => createBeep(659, 0.08, 0.08), 60);
  setTimeout(() => createBeep(784, 0.12, 0.1), 120);
};
...
const playWinSound = () => {
  ...
  setTimeout(() => createBeep(1047, 0.3, 0.12), 300);
};
...
const SprintRunner = ({ onBack }: SprintRunnerProps) => {
  ...
  useEffect(() => {
    ...
    while (x < GAME_WIDTH + 100) {
      ...
      initialGround.push({ x, height: 2 + Math.random() * 6, width });
      x += width;
    }
  ...
  const spawnObject = useCallback(() => {
    ...
    return { x: spawnX, y, width: 30, height: 30, type };
  }, []);
  ...
  const gameLoop = useCallback((timestamp: number) => {
    ...
    setGroundSegments(prev => {
      const newSegments = prev.map(seg => ({ ...seg, x: seg.x - speedRef.current * deltaTime }));
      ...
      if (rightmost < GAME_WIDTH + 50) {
        filtered.push({ x: rightmost, height: 2 + Math.random() * 6, width: 20 + Math.random() * 40 });
      }
      return filtered;
    });
  ...
  const LightbulbIcon = ({ style = {} }: { style?: React.CSSProperties }) => (
    <svg viewBox="0 0 40 40" className="w-full h-full" style={style}>
      ...
      <ellipse cx="20" cy="16" rx="11" ry="12" fill="url(#bulbGradientSR)" />
      ...
      <defs>
        <radialGradient id="bulbGradientSR" cx="50%" cy="40%" r="60%">
  ...
  const HippoIcon = ({ style = {} }: { style?: React.CSSProperties }) => (
    <svg viewBox="0 0 40 40" className="w-full h-full" style={style}>
  ...
  const renderIcon = (type: GameObject["type"]) => {
    switch (type) {
      case "bug": return <BugIcon style={{ animation: 'wiggle 0.3s ease-in-out infinite' }} />;
      case "hippo": return <HippoIcon style={{ animation: 'hippo-bob 0.5s ease-in-out infinite' }} />;
      case "insight": return <LightbulbIcon style={{ filter: 'drop-shadow(0 0 6px rgba(251, 191, 36, 0.7))', animation: 'glow-pulse 1.5s ease-in-out infinite' }} />;
    }
  };

  const gameStyles = `
    @keyframes wiggle { 0%, 100% { transform: rotate(-5deg); } 50% { transform: rotate(5deg); } }
    @keyframes hippo-bob { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-2px); } }
    @keyframes glow-pulse { 0%, 100% { filter: drop-shadow(0 0 4px rgba(251, 191, 36, 0.5)); } 50% { filter: drop-shadow(0 0 10px rgba(251, 191, 36, 0.9)); } }
  `;
  ...
  const RunningPerson = ({ bobOffset = 0, rotation = 0 }: { bobOffset?: number; rotation?: number }) => {
    ...
    return (
      <svg viewBox="0 0 40 40" className="w-full h-full" style={{ transform: `translateY(${bobOffset}px) rotate(${rotation}deg)` }}>
        <circle cx="20" cy="8" r="6" fill="#6b7280" />
        <line x1="20" y1="14" x2="20" y2="26" stroke="#6b7280" strokeWidth="3" strokeLinecap="round" />
        <line x1="20" y1="18" x2={16 - armSwing * 0.15} y2={24 + Math.abs(armSwing) * 0.1} stroke="#6b7280" strokeWidth="2.5" strokeLinecap="round" />
        <line x1="20" y1="18" x2={24 + armSwing * 0.15} y2={24 - Math.abs(armSwing) * 0.1} stroke="#6b7280" strokeWidth="2.5" strokeLinecap="round" />
        <line x1="20" y1="26" x2={16 + legSwing * 0.3} y2={38} stroke="#6b7280" strokeWidth="3" strokeLinecap="round" />
        <line x1="20" y1="26" x2={24 - legSwing * 0.3} y2={38} stroke="#6b7280" strokeWidth="3" strokeLinecap="round" />
      </svg>
    );
  };
  ...
  return (
    <div>
      ...
      <div className="flex justify-center">
        ...
        >
          {showLevelUp && (
            <div className="absolute inset-0 pointer-events-none z-20" style={{ boxShadow: 'inset 0 0 30px 10px rgba(228, 137, 129, 0.5)', animation: 'pulse 0.3s ease-out' }} />
          )}
          ...
          {clouds.map((cloud, index) => (
            <div key={`cloud-${index}`} className="absolute transition-opacity duration-1000" style={{ left: cloud.x * scaleFactor, top: cloud.y * scaleFactor, width: cloud.size * scaleFactor, height: cloud.size * 0.6 * scaleFactor, opacity: theme.cloudOpacity }}>
          ...
          {gameState === "playing" && (
            <div className="absolute top-4 left-4 flex items-center gap-2">
              <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium transition-all duration-300 ${showLevelUp ? 'bg-coral text-white scale-110' : 'bg-slate-200 text-slate-600'}`}>
                ⚡ Sprint {level}
              </div>
            </div>
          )}
          ...
          {gameState !== "start" && (
            <div className="absolute top-4 right-4 text-sm font-medium text-slate-600 text-right">
              <div>Users: <span className="text-slate-800 font-bold">{Math.floor(score)}</span></div>
              {highScore > 0 && <div className="text-xs text-slate-500">Best: {highScore}</div>}
            </div>
          )}

          {showHint && !hasJumpedThisGame && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="absolute z-30 flex items-center gap-2" style={{ left: 60 * scaleFactor, top: (playerY - 50) * scaleFactor }}>
              <div className="bg-white/90 backdrop-blur-sm rounded-lg px-3 py-2 shadow-lg text-sm font-medium text-slate-700">Tap or Space to Jump!</div>
              <motion.div animate={{ y: [0, 5, 0] }} transition={{ repeat: Infinity, duration: 0.8 }} className="text-coral">↓</motion.div>
            </motion.div>
          )}

          {particles.map(particle => (
            <div key={particle.id} className="absolute rounded-full pointer-events-none" style={{ left: particle.x * scaleFactor, top: particle.y * scaleFactor, width: (particle.type === 'dust' ? 3 : 6) * scaleFactor, height: (particle.type === 'dust' ? 3 : 6) * scaleFactor, backgroundColor: particle.color, opacity: particle.life / particle.maxLife, transform: particle.type === 'sparkle' ? 'rotate(45deg)' : undefined }} />
          ))}

          <div className="absolute left-0 right-0" style={{ top: GROUND_Y * scaleFactor }}>
            {groundSegments.map((seg, index) => (
              <div key={`ground-${index}`} className="absolute bg-slate-400" style={{ left: seg.x * scaleFactor, top: (-seg.height / 2) * scaleFactor, width: seg.width * scaleFactor, height: seg.height * scaleFactor, borderRadius: '1px' }} />
            ))}
            <div className="absolute left-0 right-0 h-0.5 bg-slate-400" />
          </div>

          {trailPositions.map((trail, index) => (
            <div key={`trail-${index}`} className="absolute pointer-events-none" style={{ left: (trail.x - index * 8) * scaleFactor, top: trail.y * scaleFactor, width: PLAYER_SIZE * scaleFactor, height: PLAYER_SIZE * scaleFactor, opacity: trail.opacity * 0.3 }}>
              <RunningPerson bobOffset={0} rotation={jumpRotation} />
            </div>
          ))}

          <div className="absolute" style={{ left: 60 * scaleFactor, top: playerY * scaleFactor, width: PLAYER_SIZE * scaleFactor, height: PLAYER_SIZE * scaleFactor, willChange: 'transform', transform: 'translateZ(0)' }}>
            <RunningPerson bobOffset={bobOffset} rotation={jumpRotation} />
          </div>

          {objects.map((obj, index) => (
            <div key={index} className="absolute" style={{ left: obj.x * scaleFactor, top: obj.y * scaleFactor, width: obj.width * scaleFactor, height: obj.height * scaleFactor, willChange: 'transform', transform: 'translateZ(0)' }}>
              {renderIcon(obj.type)}
            </div>
          ))}
          ...
          {gameState === "start" && !isLoading && (
            <div className="absolute inset-0 bg-background/90 flex flex-col items-center justify-center z-10 gap-4">
              <Button onClick={startGame} className="bg-primary hover:bg-primary/90 px-8 py-3 text-lg" aria-label="Start the game">Start Game</Button>
